...
#define FAIL(name)                                                             \
  return reinterpret_cast<Object *>(                                           \
      ((Program::name##_INDEX) << Error::ERROR_SHIFT) | Error::ERROR_TAG)
...
#define MODULE_DHCP(PRIMITIVE) PRIMITIVE(wait_for_lwip_dhcp_on_linux, 0)
...
#define MODULE_CRYPTO(PRIMITIVE)                                               \
  ...
  PRIMITIVE(rsa_create_key_pair, 2)                                            \
  PRIMITIVE(rsa_export_private_key, 1)                                         \
  PRIMITIVE(rsa_export_public_key, 1)                                          \
  PRIMITIVE(rsa_encrypt, 4)                                                    \
  PRIMITIVE(rsa_decrypt, 4)

#define MODULE_CRYPTO_RANDOM(PRIMITIVE) PRIMITIVE(random, 1)
...
#define MODULE_SNAPSHOT(PRIMITIVE) PRIMITIVE(launch, 4)
...
#define MODULE_DEBUG(PRIMITIVE) PRIMITIVE(object_histogram, 2)
...
#define MODULE_IMPLEMENTATION(name, entries)                                   \
  entries(MODULE_IMPLEMENTATION_PRIMITIVE) static const PrimitiveEntry         \
      name##_primitive_table[] = {entries(MODULE_IMPLEMENTATION_ENTRY)};       \
...
#define _A_T_LargeInteger(N, name)                                             \
  __ARG__(N, name, LargeInteger, is_large_integer)
...
#define _A_T_int8(N, name)                                                     \
  ...
  if (INT8_MIN > _value_##name || _value_##name > INT8_MAX)                    \
    FAIL(OUT_OF_RANGE);                                                        \
...
#define _A_T_uint8(N, name)                                                    \
  ...
  word _value_##name = Smi::value(_raw_##name);                                \
  if (0 > _value_##name || _value_##name > UINT8_MAX)                          \
    FAIL(OUT_OF_RANGE);                                                        \
...
#define _A_T_int16(N, name)                                                    \
  ...
  word _value_##name = Smi::value(_raw_##name);                                \
  if (INT16_MIN > _value_##name || _value_##name > INT16_MAX)                  \
    FAIL(OUT_OF_RANGE);                                                        \
...
#define _A_T_uint16(N, name)                                                   \
  ...
  word _value_##name = Smi::value(_raw_##name);                                \
  if (0 > _value_##name || _value_##name > UINT16_MAX)                         \
    FAIL(OUT_OF_RANGE);                                                        \
...
#define _A_T_int32(N, name)                                                    \
  ...
  if (_value_##name < INT32_MIN || _value_##name > INT32_MAX)                  \
    FAIL(OUT_OF_RANGE);                                                        \
...
#define _A_T_cstring(N, name)                                                  \
  ...
  if (_raw_##name != process->null_object()) {                                 \
    Blob _blob_##name;                                                         \
    if (!_raw_##name->byte_content(process->program(), &_blob_##name,          \
                                   STRINGS_ONLY))                              \
      FAIL(WRONG_OBJECT_TYPE);                                                 \
    ...
    _nonconst_##name =                                                         \
        unvoid_cast<char *>(calloc(_blob_##name.length() + 1, 1));             \
    if (!_nonconst_##name)                                                     \
      FAIL(MALLOC_FAILED);                                                     \
...
#define _A_T_CStringBlob(N, name)                                              \
  Object *_raw_##name = __args[-(N)];                                          \
  Blob name;                                                                   \
  if (!_raw_##name->byte_content(process->program(), &name, STRINGS_ONLY))     \
    FAIL(WRONG_OBJECT_TYPE);                                                   \
  if (memchr(name.address(), '\0', name.length()) != null)                     \
    FAIL(INVALID_ARGUMENT);
...
#define _A_T_blob_or_string_with_terminating_null(N, name)                     \
  ...
  if (is_string(_raw_##name)) {                                                \
    ...
    } else if (_raw_##name->byte_content(process->program(), &_blob_##name,    \
...
Object *get_absolute_path(Process *process, const wchar_t *pathname,
                          wchar_t *output,
                          const wchar_t *used_for_relative = null);

#define _A_T_WindowsPath(N, name)                                              \
  Object *_raw_##name = __args[-(N)];                                          \
  Blob name##_blob;                                                            \
  if (!_raw_##name->byte_content(process->program(), &name##_blob,             \
                                 STRINGS_ONLY))                                \
    FAIL(WRONG_OBJECT_TYPE);                                                   \
...
#define _A_T_Blob(N, name)                                                     \
  ...
  Blob name(_u_##name);                                                        \
  if (!_raw_##name->byte_content(process->program(), &name,                    \
                                 STRINGS_OR_BYTE_ARRAYS))                      \
    FAIL(WRONG_BYTES_TYPE);

#define _A_T_MutableBlob(N, name)                                              \
  Object *_raw_##name = __args[-(N)];                                          \
  MutableBlob name;                                                            \
  Error *_mutable_blob_error_##name;                                           \
  if (!_raw_##name->mutable_byte_content(process, &name,                       \
                                         &_mutable_blob_error_##name))         \
    return _mutable_blob_error_##name;
...
#define _A_T_SimpleResourceGroup(N, name)                                      \
  MAKE_UNPACKING_MACRO(SimpleResourceGroup, N, name)
#define _A_T_DacResourceGroup(N, name)                                         \
  MAKE_UNPACKING_MACRO(DacResourceGroup, N, name)
#define _A_T_GpioResourceGroup(N, name)                                        \
  MAKE_UNPACKING_MACRO(GpioResourceGroup, N, name)
#define _A_T_TouchResourceGroup(N, name)                                       \
  MAKE_UNPACKING_MACRO(TouchResourceGroup, N, name)
#define _A_T_I2cResourceGroup(N, name)                                         \
  MAKE_UNPACKING_MACRO(I2cResourceGroup, N, name)
#define _A_T_I2sResourceGroup(N, name)                                         \
  MAKE_UNPACKING_MACRO(I2sResourceGroup, N, name)
#define _A_T_PersistentResourceGroup(N, name)                                  \
  MAKE_UNPACKING_MACRO(PersistentResourceGroup, N, name)
#define _A_T_PipeResourceGroup(N, name)                                        \
  MAKE_UNPACKING_MACRO(PipeResourceGroup, N, name)
#define _A_T_SubprocessResourceGroup(N, name)                                  \
  MAKE_UNPACKING_MACRO(SubprocessResourceGroup, N, name)
...
#define _A_T_SpiResourceGroup(N, name)                                         \
  MAKE_UNPACKING_MACRO(SpiResourceGroup, N, name)
#define _A_T_SpiFlashResourceGroup(N, name)                                    \
  MAKE_UNPACKING_MACRO(SpiFlashResourceGroup, N, name)
#define _A_T_SignalResourceGroup(N, name)                                      \
  MAKE_UNPACKING_MACRO(SignalResourceGroup, N, name)
#define _A_T_SocketResourceGroup(N, name)                                      \
  MAKE_UNPACKING_MACRO(SocketResourceGroup, N, name)
#define _A_T_TcpResourceGroup(N, name)                                         \
  MAKE_UNPACKING_MACRO(TcpResourceGroup, N, name)
#define _A_T_MbedTlsResourceGroup(N, name)                                     \
  MAKE_UNPACKING_MACRO(MbedTlsResourceGroup, N, name)
#define _A_T_TimerResourceGroup(N, name)                                       \
  MAKE_UNPACKING_MACRO(TimerResourceGroup, N, name)
#define _A_T_UdpResourceGroup(N, name)                                         \
  MAKE_UNPACKING_MACRO(UdpResourceGroup, N, name)
#define _A_T_UartResourceGroup(N, name)                                        \
  MAKE_UNPACKING_MACRO(UartResourceGroup, N, name)
#define _A_T_WifiResourceGroup(N, name)                                        \
  MAKE_UNPACKING_MACRO(WifiResourceGroup, N, name)
#define _A_T_EthernetResourceGroup(N, name)                                    \
  MAKE_UNPACKING_MACRO(EthernetResourceGroup, N, name)
#define _A_T_BleResourceGroup(N, name)                                         \
  MAKE_UNPACKING_MACRO(BleResourceGroup, N, name)
#define _A_T_X509ResourceGroup(N, name)                                        \
  MAKE_UNPACKING_MACRO(X509ResourceGroup, N, name)
#define _A_T_PwmResourceGroup(N, name)                                         \
  MAKE_UNPACKING_MACRO(PwmResourceGroup, N, name)
#define _A_T_RpcResourceGroup(N, name)                                         \
  MAKE_UNPACKING_MACRO(RpcResourceGroup, N, name)
#define _A_T_RmtResourceGroup(N, name)                                         \
  MAKE_UNPACKING_MACRO(RmtResourceGroup, N, name)
#define _A_T_PcntUnitResourceGroup(N, name)                                    \
  MAKE_UNPACKING_MACRO(PcntUnitResourceGroup, N, name)
#define _A_T_EspNowResourceGroup(N, name)                                      \
  MAKE_UNPACKING_MACRO(EspNowResourceGroup, N, name)
...
#define _A_T_ImageOutputStream(N, name)                                        \
  MAKE_UNPACKING_MACRO(ImageOutputStream, N, name)
#define _A_T_I2cBusResource(N, name)                                           \
  MAKE_UNPACKING_MACRO(I2cBusResource, N, name)
#define _A_T_I2cDeviceResource(N, name)                                        \
  MAKE_UNPACKING_MACRO(I2cDeviceResource, N, name)
...
#define _A_T_EthernetEvents(N, name)                                           \
  MAKE_UNPACKING_MACRO(EthernetEvents, N, name)
#define _A_T_EthernetIpEvents(N, name)                                         \
  MAKE_UNPACKING_MACRO(EthernetIpEvents, N, name)
#define _A_T_MbedTlsSocket(N, name) MAKE_UNPACKING_MACRO(MbedTlsSocket, N, name)
#define _A_T_BaseMbedTlsSocket(N, name)                                        \
  MAKE_UNPACKING_MACRO(BaseMbedTlsSocket, N, name)
#define _A_T_X509Certificate(N, name)                                          \
  MAKE_UNPACKING_MACRO(X509Certificate, N, name)
...
#define _A_T_GpioPinResource(N, name)                                          \
  MAKE_UNPACKING_MACRO(GpioPinResource, N, name)
#define _A_T_GpioChipResource(N, name)                                         \
  MAKE_UNPACKING_MACRO(GpioChipResource, N, name)
#define _A_T_UartResource(N, name) MAKE_UNPACKING_MACRO(UartResource, N, name)
#define _A_T_UdpSocketResource(N, name)                                        \
  MAKE_UNPACKING_MACRO(UdpSocketResource, N, name)
#define _A_T_TcpSocketResource(N, name)                                        \
  MAKE_UNPACKING_MACRO(TcpSocketResource, N, name)
#define _A_T_TcpServerSocketResource(N, name)                                  \
  MAKE_UNPACKING_MACRO(TcpServerSocketResource, N, name)
#define _A_T_SubprocessResource(N, name)                                       \
  MAKE_UNPACKING_MACRO(SubprocessResource, N, name)
#define _A_T_ReadPipeResource(N, name)                                         \
  MAKE_UNPACKING_MACRO(ReadPipeResource, N, name)
#define _A_T_WritePipeResource(N, name)                                        \
  MAKE_UNPACKING_MACRO(WritePipeResource, N, name)
...
#define _A_T_PmLockResource(N, name)                                           \
  MAKE_UNPACKING_MACRO(PmLockResource, N, name)
#define _A_T_PwmResource(N, name) MAKE_UNPACKING_MACRO(PwmResource, N, name)
#define _A_T_PcntUnitResource(N, name)                                         \
  MAKE_UNPACKING_MACRO(PcntUnitResource, N, name)
#define _A_T_EspNowResource(N, name)                                           \
  MAKE_UNPACKING_MACRO(EspNowResource, N, name)
#define _A_T_RmtResource(N, name) MAKE_UNPACKING_MACRO(RmtResource, N, name)
#define _A_T_RmtSyncManagerResource(N, name)                                   \
  MAKE_UNPACKING_MACRO(RmtSyncManagerResource, N, name)
#define _A_T_RmtSyncManagerResource(N, name)                                   \
  MAKE_UNPACKING_MACRO(RmtSyncManagerResource, N, name)
#define _A_T_RmtPatternEncoderResource(N, name)                                \
  MAKE_UNPACKING_MACRO(RmtPatternEncoderResource, N, name)
#define _A_T_BleResource(N, name) MAKE_UNPACKING_MACRO(BleResource, N, name)
#define _A_T_BleAdapterResource(N, name)                                       \
  MAKE_UNPACKING_MACRO(BleAdapterResource, N, name)
#define _A_T_BleReadWriteElement(N, name)                                      \
  MAKE_UNPACKING_MACRO(BleReadWriteElement, N, name)
#define _A_T_BleCallbackResource(N, name)                                      \
  MAKE_UNPACKING_MACRO(BleCallbackResource, N, name)
#define _A_T_BleCentralManagerResource(N, name)                                \
  MAKE_UNPACKING_MACRO(BleCentralManagerResource, N, name)
#define _A_T_BleRemoteDeviceResource(N, name)                                  \
  MAKE_UNPACKING_MACRO(BleRemoteDeviceResource, N, name)

#define _A_T_BlePeripheralManagerResource(N, name)                             \
  MAKE_UNPACKING_MACRO(BlePeripheralManagerResource, N, name)
#define _A_T_BleCharacteristicResource(N, name)                                \
  MAKE_UNPACKING_MACRO(BleCharacteristicResource, N, name)
#define _A_T_BleServiceResource(N, name)                                       \
  MAKE_UNPACKING_MACRO(BleServiceResource, N, name)
#define _A_T_BleServerConfigGroup(N, name)                                     \
  MAKE_UNPACKING_MACRO(BleServerConfigGroup, N, name)
#define _A_T_BleServerServiceResource(N, name)                                 \
  MAKE_UNPACKING_MACRO(BleServerServiceResource, N, name)
#define _A_T_BleServerCharacteristicResource(N, name)                          \
  MAKE_UNPACKING_MACRO(BleServerCharacteristicResource, N, name)
#define _A_T_ServiceDescription(N, name)                                       \
  MAKE_UNPACKING_MACRO(ServiceDescription, N, name)
...
// ARGS is expanded to one of the following depending on number of passed
// parameters.
#define _ODD ARGS cannot take odd number of arguments

#define _A_2(t1, n1) _A_T_##t1(0, n1);
...
#define _A_18(t1, n1, t2, n2, t3, n3, t4, n4, t5, n5, t6, n6, t7, n7, t8, n8,  \
              t9, n9)                                                          \
  _A_T_##t1(0, n1);                                                            \
  _A_T_##t2(1, n2);                                                            \
  _A_T_##t3(2, n3);                                                            \
...
#define _A_20(t1, n1, t2, n2, t3, n3, t4, n4, t5, n5, t6, n6, t7, n7, t8, n8,  \
              t9, n9, t10, n10)                                                \
  _A_T_##t1(0, n1);                                                            \
  _A_T_##t2(1, n2);                                                            \
  _A_T_##t3(2, n3);                                                            \
...
#define _A_22(t1, n1, t2, n2, t3, n3, t4, n4, t5, n5, t6, n6, t7, n7, t8, n8,  \
...
#define _A_24(t1, n1, t2, n2, t3, n3, t4, n4, t5, n5, t6, n6, t7, n7, t8, n8,  \
...
#define _A_26(t1, n1, t2, n2, t3, n3, t4, n4, t5, n5, t6, n6, t7, n7, t8, n8,  \
              t9, n9, t10, n10, t11, n11, t12, n12, t13, n13)                  \
  _A_T_##t1(0, n1);                                                            \
  _A_T_##t2(1, n2);                                                            \
  _A_T_##t3(2, n3);                                                            \
  _A_T_##t4(3, n4);                                                            \
...
#define _OVERRIDE(_1, _2, _3, _4, _5, _6, _7, _8, _9, _10, _11, _12, _13, _14, \
                  _15, _16, _17, _18, _19, _20, _21, _22, _23, _24, _25, _26,  \
                  NAME, ...)                                                   \
  NAME

#define ARGS(...)                                                              \
  _OVERRIDE(__VA_ARGS__, _A_26, _ODD, _A_24, _ODD, _A_22, _ODD, _A_20, _ODD,   \
            _A_18, _ODD, _A_16, _ODD, _A_14, _ODD, _A_12, _ODD, _A_10, _ODD,   \
            _A_8, _ODD, _A_6, _ODD, _A_4, _ODD, _A_2, _ODD)                    \
  (__VA_ARGS__)
...
#define PRIVILEGED                                                             \
  if (!process->is_privileged())                                               \
    return Primitive::mark_as_error(process->program()->privileged_primitive());
