...
PRIMITIVE(rsa_create_key_pair) {
  ...
  int ret = mbedtls_pk_setup(rsa->context(), mbedtls_pk_info_from_type(MBEDTLS_PK_RSA));
  if (ret != 0) {
    delete rsa;
    return tls_error(null, process, ret);
  }

  ret = mbedtls_rsa_gen_key(mbedtls_pk_rsa(*(rsa->context())), rsa_rng, NULL, bits, 65537);
  if (ret != 0) {
    delete rsa;
    return tls_error(null, process, ret);
  }
...
static Object* rsa_export_key_helper(RsaKey* rsa, Process* process, bool is_private) {
